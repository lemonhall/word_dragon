...
| `REQ-0001-002` | `PRD-0001 §REQ-0001-002` | `v1-content-pipeline`、`v1-manual-idiom-curation` | `tools/content_pipeline/tests/test_catalog_filter.py`、`tools/content_pipeline/tests/test_manual_review.py` | — | `uv run pytest tools/content_pipeline/tests/test_catalog_filter.py -q`；`uv run pytest tools/content_pipeline/tests/test_manual_review.py -q` | `2026-03-31`：`idiom_catalog.json` 生成 `36084` 条启用词条，全部为四字成语；`2026-04-01`：`M5` 工作区已冻结为 `181` 批，`research/content/manual_review/m5` 已生成 `progress.json`、`source_hashes.json` 与批次源文件；`tools/content_pipeline/tests/test_manual_review.py -q` 当前 `20 passed` | doing |
...
- `M5` 进行中：`2026-04-01` 已冻结 `36084` 条启用词条为 `181` 批真实审核工作区，`validate_review_batch` 与失败分支测试已落地并推送；下一步继续实现 `open-next / close-batch / assemble`，然后进入逐批人工审核。
